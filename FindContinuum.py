...
def GetContinuumRegions(x, y, cont=None, lowreject=3, highreject=3):
    ...
    while not done:
        ...
        badpoints = np.where(np.logical_or(normalized2 - 1.0 < -lowreject * std, normalized2 - 1.0 > highreject * std))[
            0]
        goodpoints = np.where(np.logical_and(normalized - 1.0 > -lowreject * std, normalized - 1.0 < highreject * std))[
            0]
