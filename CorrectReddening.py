...
def FitFunction(colors, SpT, distance, Rv=3.1):
    ...
    for color in colors:
        if colors[color][0] != None:
            ...
            print "%g +/- %g\n%g" % (
            colors[color][0], colors[color][1], extinct.Alambda(Color2Lambda[color]) + 5.0 * np.log10(pars[2]) - 5.0)
...
def ErrorFunction(pars, colors, SpT):
    ...
    for color in colors:
        if colors[color][0] != None:
            difference += (colors[color][0] - extinct.Alambda(Color2Lambda[color]) - 5.0 * np.log10(d) + 5.0) ** 2 / \
                          colors[color][1] ** 2
