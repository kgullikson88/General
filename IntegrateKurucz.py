...
class Integrate:
    def __init__(self, kurucz=None, Tvals=np.arange(5000, 30000, 500),
        ...
        for T in Tvals:
            ...
            for logg in gvals:
                for metal in metalvals:
                    ...
                    if fluxtype == "Fnu":
                        spectrum.y *= Units.angstrom / Units.cm * (
                                                                  spectrum.x * Units.cm / Units.nm) ** 2 / Units.c * unitconv
        ...
        self.Interpolators = {}
        for color in Fluxes:
            self.Interpolators[color] = lambda Temp, grav, feonh, fluxes=Fluxes[color]: griddata(
                (Temperatures, Gravities, Metallicities), fluxes, (Temp, grav, feonh), method='linear')
